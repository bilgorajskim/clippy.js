export class Animator {
  ...
  hasAnimation(name) { return !!this._data.animations[name]; }

  exitAnimation() { this._exiting = true; }
  ...
  _step() {
    ...
    // always switch frame data, unless we're at the last frame of an animation
    // with a useExitBranching flag.
    ...
    this._loop = window.setTimeout(
        $.proxy(this._step, this), this._currentFrame.duration);
    ...
    if (this._endCallback && frameChanged && this._atLastFrame()) {
      if (this._currentAnimation.useExitBranching && !this._exiting) {
        this._endCallback(
            this.currentAnimationName, clippy.Animator.States.WAITING);
      } else {
        this._endCallback(
            this.currentAnimationName, clippy.Animator.States.EXITED);
      }
    }
  }
   ...
   */
  pause() { window.clearTimeout(this._loop); }
  ...
  static States() { return {WAITING: 1, EXITED: 0}; }
}
